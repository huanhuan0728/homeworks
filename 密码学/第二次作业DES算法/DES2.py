...
class DES(object):
    ...
    def F(self, block, subKeyid):
        ...
        print("32位输入:", format(block, '032b'))

        expanded_block = Permute(block, 32, E)
        print("选择运算:", format(expanded_block, '048b'))

        temp = Permute(block, 32, E) ^ self.subKs[subKeyid]

        print("子密钥K_{}:".format(subKeyid + 1), format(self.subKs[subKeyid], '048b'))
        print("子密钥加:", format(temp, '048b'))
        ...
        for i in range(8):
            ...
            # 作业三第二题
            # if i == 0:  # 只关注第一个S盒
            #     input_to_sbox = yxxxxy  # 第一个S盒的输入
            #     output_from_sbox = S[i][yy][xxxx]  # 第一个S盒的输出
            #     print(f"S盒1 输入: {format(input_to_sbox, '06b')}, 输出: {format(output_from_sbox, '04b')}")

            sbox_output = S[i][yy][xxxx]
            # res |= S[i][yy][xxxx]
            res |= sbox_output
            if i < 7:  # 除了最后一轮，每次将res左移4位以便于下一个S盒的输出
                res = res << 4

        # 循环结束后，res包含了所有S盒的输出
        print("S盒输出:", format(res, '032b'))

        res = Permute(res, 32, P)
        print("P置换:", format(res, '032b'))
    ...
    def Encrypt(self, m: bytes) -> bytes:
        ...
        for i in range(len(blocks) // 2):
            ...
            print("初始置换：", format(temp, '064b'))
            print("L0:", format(high, '032b'))
            print("R0:", format(low, '032b'))

            # 第三步：共16轮迭代
            for j in range(16):
                # 在处理本轮之前打印L和R的当前值
                # print(f"第 {j + 1} 轮开始: L = {format(high, '032b')}, R = {format(low, '032b')}")
                ...
                temp_high = low
                temp_low = high ^ self.F(low, j)
                # 在处理本轮之后打印L和R的新值
                # print(f"第 {j + 1} 轮结束: L = {format(temp_low, '032b')}, R = {format(temp_high, '032b')}")
                print("L{}:".format(j + 1), format(high, '032b'))
                print("R{}:".format(j + 1), format(low, '032b'))
                if j == 15:  # 第16轮结束后
                    high, low = low, high  # 交换L16和R16
                    final_permutation = Permute((high << 32) | low, 64, FP) & 0xffffffffffffffff
                    print("逆初始置换:", format(final_permutation, '064b'))

                high, low = temp_high, temp_low
    ...
    def generate_subKs(self):
        print("1. 密钥拓展")
        print("密钥：", end='')
        print_bytes_hex(self.K)  # 假设已定义的打印函数
        ...
        print("置换选择1:")
        print("C0:", format(C, '028b'))
        print("D0:", format(D, '028b'))
        ...
        for i in range(16):
            ...
            print(f"\nN = {i + 1}")
            print("C{}:".format(i + 1), format(C, '028b'))
            print("D{}:".format(i + 1), format(D, '028b'))
            print("子密钥K{}:".format(i + 1), format(self.subKs[i], '048b'))
...
def count_different_bits(bin_str1, bin_str2):
    """
    计算两个二进制字符串之间不同位的数量。
    :param bin_str1: 第一个二进制字符串。
    :param bin_str2: 第二个二进制字符串。
    :return: 不同位的数量。
    """
    # 确保两个字符串长度相同
    if len(bin_str1) != len(bin_str2):
        raise ValueError("二进制字符串必须长度相同")
...
def homework3_1():
    ...
    cc2 = a.Encrypt(m2)
    mm = a.Decrypt(cc)
    str1 = '1101101010110010000010110110101101100110111111100011011010100101'
    str2 = '0110110110101001111110111000000000100101111101010000100011010001'
    different = count_different_bits(str1, str2)
    print(different)
